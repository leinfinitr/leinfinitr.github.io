---
title: "Inside vLLM - 01: How Model Weights Reach GPU Memory"
...
Therefore `memory_reserved() >= memory_allocated()`, and `nvidia-smi` may remain high after a tensor is deleted. `empty_cache()` returns **unused cached blocks** to CUDA, but it cannot free storage still owned by a live tensor.[[3](https://docs.pytorch.org/docs/stable/notes/cuda.html)]
...
`CUDAPluggableAllocator` loads two exported functions from a shared library:[[3](https://docs.pytorch.org/docs/stable/notes/cuda.html)][[5](https://docs.pytorch.org/docs/stable/generated/torch.cuda.memory.CUDAPluggableAllocator.html)]
...
PyTorch can install such an allocator globally with `change_current_allocator()`, but that is process-wide and must happen before the existing allocator is initialized. More importantly, a global swap would route unrelated tensors through the custom policy.[[3](https://docs.pytorch.org/docs/stable/notes/cuda.html)]

### 3.2 `MemPool`: custom allocation without a global swap

`torch.cuda.MemPool` is a private pool managed by the caching allocator. It can be backed by a custom allocator, while `torch.cuda.use_mem_pool(pool)` routes only allocations in the current thread and context to that pool.[[3](https://docs.pytorch.org/docs/stable/notes/cuda.html)][[6](https://raw.githubusercontent.com/pytorch/pytorch/v2.13.0/torch/cuda/memory.py)][[7](https://docs.pytorch.org/docs/stable/generated/torch.cuda.memory.MemPool.html)]
...
`MemPool` also preserves useful caching behavior around the custom system allocator. This detail matters for the measured numbers later: the custom allocator sees **segment-sized** requests, not one request for every `nn.Parameter`.[[3](https://docs.pytorch.org/docs/stable/notes/cuda.html)]

## 4. vLLM's `CuMemAllocator`

vLLM connects that PyTorch extension point to CUDA VMM in two pieces:

- `vllm/device_allocator/cumem.py` owns Python bookkeeping, allocation tags, and sleep/wake operations.[[8](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)]
- `csrc/cumem_allocator.cpp` exports `my_malloc` and `my_free`, and calls the CUDA driver.[[9](https://github.com/vllm-project/vllm/blob/v0.27.0/csrc/cumem_allocator.cpp)]

### 4.1 Allocation: create a stable address and map VRAM into it

For a new segment, `my_malloc` performs the following steps:[[9](https://github.com/vllm-project/vllm/blob/v0.27.0/csrc/cumem_allocator.cpp)]
...
The Python callback stores an `AllocationData` record in `pointer_to_data`, keyed by the virtual address. Each record contains the CUDA handle, a **tag**, an optional CPU backup tensor, and an `is_asleep` flag.[[8](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)]

### 4.2 Tags are allocation policy, not tensor metadata

`CuMemAllocator.use_memory_pool(tag)` sets `current_tag`, creates a pluggable allocator and `MemPool`, and enters `torch.cuda.memory.use_mem_pool`. Every **new pool segment** requested inside that scope is recorded with the current tag.[[8](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)]
...
On sleep, vLLM can copy selected allocations to CPU, call `cuMemUnmap`, and release their physical handles. The virtual address reservation—and therefore every tensor pointer—remains known. Wake-up creates new physical memory and maps it back at the same address before copying saved bytes back.[[8](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)][[9](https://github.com/vllm-project/vllm/blob/v0.27.0/csrc/cumem_allocator.cpp)]
...
physical VRAM backing:  mapped ────────── released ────────── mapped
...
The key scope is only a few lines in `GPUWorker.load_model`:[[10](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/v1/worker/gpu_worker.py)]
...
When Sleep Mode is enabled, model configuration automatically enables the CuMem allocator. `_maybe_get_memory_pool_context("weights")` then returns `CuMemAllocator.use_memory_pool(tag="weights")`; otherwise it returns a no-op context.[[10](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/v1/worker/gpu_worker.py)]

Everything below happens inside that scope.

### Phase 1 — Construct the runtime model directly on CUDA

`BaseModelLoader.load_model` selects the target CUDA device and default dtype, then calls `initialize_model` inside the device context.[[11](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/base_loader.py)]
...
The default safetensors strategy opens each checkpoint with `safe_open(..., framework="pt")` and yields one CPU tensor at a time. On a local filesystem, this is lazy, memory-mapped loading rather than an eager copy of the entire file into anonymous CPU RAM.[[13](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/weight_utils.py)]
...
`Qwen3_5ForConditionalGeneration.load_weights` delegates to `AutoWeightsLoader`. The loader recursively follows module and parameter names, applies Qwen3.5's name/packing mapper, and selects a parameter-specific `weight_loader` when present.[[12](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/models/qwen3_5.py)][[14](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/models/utils.py)]
...
The source tensor is CPU-backed; `param` already points into the `weights` MemPool on CUDA. `copy_` transfers bytes into that existing address. It does **not** replace the parameter or allocate another full-sized CUDA destination.[[13](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/weight_utils.py)]
...
After all checkpoint tensors are copied, vLLM validates that required parameters were loaded, runs quantization- and attention-specific post-load hooks, and returns `model.eval()`.[[11](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/base_loader.py)][[15](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/default_loader.py)][[16](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/utils.py)]

When `GPUWorker.load_model` exits `use_memory_pool("weights")`, normal allocations return to the default PyTorch pool. The model tensors, however, retain their private-pool storage. vLLM also keeps strong references to the `MemPool` and `CUDAPluggableAllocator`, because their lifetime must exceed the tensors and any captured CUDA graphs.[[8](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)]
...
I instrumented four boundaries in the real vLLM loader and read both PyTorch allocator counters and `CuMemAllocator.pointer_to_data`. The values below are from that run.
...
> **Why is total CUDA usage higher?** The CUDA context, NCCL, kernels, and other libraries allocate memory outside PyTorch's tagged weight pool. vLLM intentionally initializes distributed state before its baseline snapshot. Those bytes remain real, but they are not model-weight storage and should not be attributed to the checkpoint.[[10](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/v1/worker/gpu_worker.py)][[17](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/utils/mem_utils.py)]
{: .prompt-info }
...
- [1] [NVIDIA: Introducing Low-Level GPU Virtual Memory Management](https://developer.nvidia.com/blog/introducing-low-level-gpu-virtual-memory-management)
- [2] [CUDA Driver API: Virtual Memory Management](https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__VA.html)
- [3] [PyTorch CUDA semantics: CUDA memory management](https://docs.pytorch.org/docs/stable/notes/cuda.html)
- [4] [A guide to PyTorch's CUDA Caching Allocator](https://zdevito.github.io/2022/08/04/cuda-caching-allocator.html)
- [5] [PyTorch `CUDAPluggableAllocator` API](https://docs.pytorch.org/docs/stable/generated/torch.cuda.memory.CUDAPluggableAllocator.html)
- [6] [PyTorch 2.13 CUDA memory Python source](https://raw.githubusercontent.com/pytorch/pytorch/v2.13.0/torch/cuda/memory.py)
- [7] [PyTorch `MemPool` API](https://docs.pytorch.org/docs/stable/generated/torch.cuda.memory.MemPool.html)
- [8] [vLLM v0.27.0 `cumem.py`](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/device_allocator/cumem.py)
- [9] [vLLM v0.27.0 CuMem C++ allocator](https://github.com/vllm-project/vllm/blob/v0.27.0/csrc/cumem_allocator.cpp)
- [10] [vLLM v0.27.0 GPU worker](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/v1/worker/gpu_worker.py)
- [11] [vLLM v0.27.0 base model loader](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/base_loader.py)
- [12] [vLLM v0.27.0 Qwen3.5 model](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/models/qwen3_5.py)
- [13] [vLLM v0.27.0 weight utilities](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/weight_utils.py)
- [14] [vLLM v0.27.0 `AutoWeightsLoader`](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/models/utils.py)
- [15] [vLLM v0.27.0 default model loader](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/default_loader.py)
- [16] [vLLM v0.27.0 model loader utilities](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/model_executor/model_loader/utils.py)
- [17] [vLLM v0.27.0 memory profiler utilities](https://github.com/vllm-project/vllm/blob/v0.27.0/vllm/utils/mem_utils.py)
